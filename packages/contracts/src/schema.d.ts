...
export interface paths {
    ...
    "/api/v1/auth/register": {
         ...
         * @description **202, and no user object** — ADR 0021.
         *
    ...
    "/api/v1/trips/{public_id}/quote": {
         ...
         *
         *     **`Idempotency-Key` is required.** Retrying with the same key replays the first answer rather than holding a second set of seats; a second request arriving while the first is still running is `409 IDEMPOTENT_REQUEST_IN_PROGRESS` and is safe to retry.
         */
...
export interface components {
    schemas: {
        ...
        /** @description What `POST /auth/register` answers with — ADR 0021.
         *
...
export interface operations {
    ...
    trips_quote_create: {
        parameters: {
            query?: never;
            header: {
                /** @description A client-generated key, at most 64 characters, unique to this attempt. Repeating a request with the same key returns the first response and takes no further capacity; reusing one for a different trip is `409 IDEMPOTENCY_KEY_REUSED`. Kept for `idempotency.retention_hours`. A failed attempt releases its key, so a retry after `409 INVENTORY_UNAVAILABLE` may reuse it. */
